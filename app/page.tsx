...
import {
  ...
  Star, Clock, Globe, Wallet, ArrowUpCircle, ArrowDownCircle,
...
const features = [
  {
    ...
    desc: "Créez des factures professionnelles en FCFA. Statuts automatiques : brouillon, en attente, payée, impayée. Corbeille avec restauration sous 30 jours.",
    color: "bg-emerald-50 text-emerald-600",
  },
  {
    icon: Package,
    title: "Stock intelligent",
    desc: "Gestion du stock avec réservation automatique à la facturation. Alertes seuil bas, import/export Excel, prix d'achat et calcul de marge en temps réel.",
    color: "bg-blue-50 text-blue-600",
  },
  {
    icon: Wallet,
    title: "Trésorerie & Transactions",
    desc: "Suivez vos entrées et sorties. Les paiements de factures sont enregistrés automatiquement. Solde net toujours à jour même après suppression de factures.",
    color: "bg-purple-50 text-purple-600",
  },
  {
    icon: PieChart,
    title: "Rapports financiers",
    desc: "Rapports par semaine, mois ou période personnalisée. Bénéfice réel vs potentiel, taux de recouvrement. Export PDF depuis n'importe quel appareil.",
    color: "bg-amber-50 text-amber-600",
  },
  {
    icon: BarChart3,
    title: "Dashboard analytique",
    desc: "CA réel depuis les transactions, taux de recouvrement visuel, top produits vendus, bénéfice potentiel du stock. Vue complète de votre activité.",
    color: "bg-rose-50 text-rose-600",
  },
  {
    icon: ShieldCheck,
    title: "Sécurisé & fiable",
    desc: "Données protégées en cloud (Supabase). Accessible depuis PC, tablette ou téléphone. Aucune donnée perdue même après suppression de factures.",
...
const steps = [
  ...
  { number: "02", title: "Ajoutez vos produits", desc: "Saisissez ou importez votre catalogue depuis Excel avec prix d'achat et de vente." },
  { number: "03", title: "Émettez vos factures", desc: "Créez une facture, recherchez vos produits, le stock se gère automatiquement." },
  { number: "04", title: "Suivez vos finances", desc: "Dashboard, rapports et trésorerie se mettent à jour à chaque paiement." },
]

const plans = [
  {
    name: "Starter", price: "Gratuit", sub: "pour toujours", accent: false, cta: "Commencer gratuitement",
    features: ["Jusqu'à 20 factures/mois", "50 produits en stock", "Transactions basiques", "Rapport mensuel", "Support par email"],
  },
  {
...
const testimonials = [
  { name: "Mahamat Saleh", role: "Gérant, Épicerie Central — N'Djamena", stars: 5, quote: "Avant, je gérais mes stocks sur papier. Maintenant tout est en temps réel. Je vois exactement ce qui se vend et ce que je gagne vraiment." },
  ...
  { name: "Idriss Brahim", role: "Directeur, Import-Export — Moundou", stars: 5, quote: "Le dashboard m'a aidé à voir que 3 produits faisaient 60% de mon CA. Et ma trésorerie est toujours à jour, même si je supprime une facture." },
]
...
const dashboardPreview = {
  ...
  invoices: [
    { name: "Facture Moussa", client: "Moussa Abakar", status: "Payée", statusColor: "bg-emerald-100 text-emerald-700", amount: "215 000 FCFA" },
    { name: "Livraison Mai", client: "SOGEC SA", status: "En attente", statusColor: "bg-amber-100 text-amber-700", amount: "480 000 FCFA" },
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-emerald-50 via-white to-cyan-50 pt-20 pb-28">
        ...
        <div className="relative max-w-6xl mx-auto px-5 md:px-8 text-center">
          <div className="inline-flex items-center gap-2 bg-emerald-100 text-emerald-700 text-xs font-semibold px-3 py-1.5 rounded-full mb-6">
            ...
            Conçu pour les PME d&apos;Afrique
          </div>
          <h1 className="text-4xl md:text-6xl font-black leading-tight tracking-tight mb-6">
            Factures, stock et finances<br />
          ...
          <p className="max-w-2xl mx-auto text-lg text-gray-500 mb-10">
            ...
            en passant par le stock et les rapports financiers. Pensé pour les commerçants et entrepreneurs d&apos;Afrique.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link href="/sign-up" className="flex items-center gap-2 px-6 py-3.5 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-xl transition-colors shadow-lg shadow-emerald-200 text-base w-full sm:w-auto justify-center">
              Créer mon compte gratuit <ArrowRight className="w-4 h-4" />
            </Link>
            <Link href="/sign-in" className="flex items-center gap-2 px-6 py-3.5 bg-white border border-gray-200 hover:border-emerald-300 text-gray-700 font-semibold rounded-xl transition-all text-base shadow-sm w-full sm:w-auto justify-center">
              J&apos;ai déjà un compte
            </Link>
          </div>
          ...
          <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-2xl shadow-gray-200 bg-white">
            ...
            <div className="p-5 md:p-7 space-y-5">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {dashboardPreview.stats.map(({ label, value, color, bg }) => (
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div className="rounded-xl border border-gray-100 overflow-hidden">
                  ...
                  {dashboardPreview.transactions.map((t, i) => (
                ...
                <div className="rounded-xl border border-gray-100 overflow-hidden">
                  ...
                  {dashboardPreview.invoices.map((inv, i) => (
                    <div key={i} className="flex items-center justify-between px-4 py-3 border-t border-gray-50">
                      <div>
                      ...
                      <div className="text-right">
                        <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${inv.statusColor}`}>{inv.status}</span>
                        <p className="text-xs font-bold mt-0.5">{inv.amount}</p>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <section id="fonctionnalites" className="py-20 bg-white">
        <div className="max-w-6xl mx-auto px-5 md:px-8">
          <div className="text-center mb-14">
            ...
            <p className="text-gray-500 max-w-xl mx-auto">Plus besoin de jongler entre Excel, WhatsApp et vos cahiers. Tout est centralisé et automatisé.</p>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-5 md:px-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              ...
              <p className="text-emerald-100 text-sm">Suivez vos entrées/sorties, générez des rapports par semaine ou mois, et exportez en PDF depuis n&apos;importe quel appareil.</p>
            </div>
            <div className="flex flex-col sm:flex-row gap-3 shrink-0">
              {[
                ...
                <div key={label} className="flex items-center gap-2 bg-white/15 backdrop-blur-sm rounded-xl px-3 py-2 text-white text-sm font-semibold">
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-6xl mx-auto px-5 md:px-8">
          <div className="text-center mb-14">
            ...
            <p className="text-gray-500 max-w-xl mx-auto">Aucune formation requise. Vous êtes opérationnel dès votre inscription.</p>
          </div>
      ...
      <section className="py-24 bg-gray-900 text-white text-center">
        <div className="max-w-3xl mx-auto px-5 md:px-8">
          ...
          <p className="text-gray-400 text-lg mb-10">
            Rejoignez les entrepreneurs qui ont arrêté les cahiers et les fichiers Excel.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link href="/sign-up" className="flex items-center gap-2 px-7 py-4 bg-emerald-500 hover:bg-emerald-400 text-white font-bold rounded-xl transition-colors text-base shadow-lg shadow-emerald-900/30 w-full sm:w-auto justify-center">
              Commencer gratuitement <ArrowRight className="w-4 h-4" />
            </Link>
            <Link href="/sign-in" className="flex items-center gap-2 px-7 py-4 border border-gray-700 hover:border-gray-500 text-gray-300 hover:text-white font-semibold rounded-xl transition-all text-base w-full sm:w-auto justify-center">
